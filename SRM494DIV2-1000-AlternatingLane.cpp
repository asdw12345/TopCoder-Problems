...
double AlternatingLane::expectedBeauty(vector <int> low, vector <int> high)
{
    ...
    for(i=0;i<n-1;i++)
    {
        ...
        for(j=low[i];j<=high[i];j++)
        {
            if(j>=high[i+1])
            {
                sum+=(high[i+1]-low[i+1]+1)*(j-(low[i+1]+high[i+1])*0.5)*1.0;
            }
            else if(j<=low[i+1])
            {
                sum+=(high[i+1]-low[i+1]+1)*((low[i+1]+high[i+1])*0.5-j)*1.0;
            }
            else
            {
                sum+=(j-low[i+1]+1)*(j-(low[i+1]+j)*0.5)*1.0+(high[i+1]-j+1)*((high[i+1]+j)*0.5-j);
            }
        }
        sum=sum*1.0/((high[i+1]-low[i+1]+1)*1.0)/((high[i]-low[i]+1)*1.0);
        ans+=sum;
    }
    return ans;
}
